...
echo "Storage test directory set up at /data/storage_tests/out$GCRNODE/$timestamp"
...
fio randwrite.fio --output-format=json --output=/data/storage-tests/$GCRNODE/$timestamp/randwrite.json
...
echo "Storage tests completed. Results saved in /data/storage-tests/$GCRNODE/$timestamp/"
